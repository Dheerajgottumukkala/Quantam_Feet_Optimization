    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$falqon$2d$fleet$2d$optimizer$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "GoogleMapComponent.useEffect": ()=>{
            ...
            const drawRoute = {
                "GoogleMapComponent.useEffect.drawRoute": ()=>{
                    ...
                    if (pythonData.route_plan && pythonData.route_plan.length >= 2) {
                        ...
                        const waypoints = coordsList.slice(1, -1).map({
                            "GoogleMapComponent.useEffect.drawRoute.waypoints": (p)=>({
                                    location: {
                                        lat: p.lat,
                                        lng: p.lon
                                    }
                                })
                        }["GoogleMapComponent.useEffect.drawRoute.waypoints"]);
                        const request = {
                            origin,
                            destination,
                            waypoints,
                    ...
                    directionsService.route(request, {
                        "GoogleMapComponent.useEffect.drawRoute": (result, status)=>{
                            if (window.google.maps.DirectionsStatus && status === window.google.maps.DirectionsStatus.OK || status === 'OK') {
                                ...
                                const coordsList = route.map({
                                    "GoogleMapComponent.useEffect.drawRoute.coordsList": (name)=>geocodedPoints[name]
                                }["GoogleMapComponent.useEffect.drawRoute.coordsList"]).filter(Boolean);
                                if (coordsList.length >= 2) {
                                    drawDirectionsBySegments(coordsList.map({
                                        "GoogleMapComponent.useEffect.drawRoute": (p)=>({
                                                name: '',
                                                lat: p.lat,
                                                lon: p.lng
                                            })
          ...
          `
            ...
            const drawPolylineFromPlan = {
                "GoogleMapComponent.useEffect.drawPolylineFromPlan": (routePlan)=>{
                    const path = routePlan.map({
                        "GoogleMapComponent.useEffect.drawPolylineFromPlan.path": (p)=>({
                                lat: p.lat,
                                lng: p.lon
                            })
                    }["GoogleMapComponent.useEffect.drawPolylineFromPlan.path"]);
                    drawPolyline(path);
                }
            }["GoogleMapComponent.useEffect.drawPolylineFromPlan"];
            const drawPolyline = {
                "GoogleMapComponent.useEffect.drawPolyline": (path)=>{
                    ...
                    path.forEach({
                        "GoogleMapComponent.useEffect.drawPolyline": (p)=>bounds.extend(p)
                    }["GoogleMapComponent.useEffect.drawPolyline"]);
                    map.fitBounds(bounds);
                }
            }["GoogleMapComponent.useEffect.drawPolyline"];
            const drawDirectionsBySegments = {
                "GoogleMapComponent.useEffect.drawDirectionsBySegments": async (routePlan)=>{
                    const allPaths = [];
                    for(let i = 0; i < routePlan.length - 1; i++){
                        ...
                        await new Promise({
                            "GoogleMapComponent.useEffect.drawDirectionsBySegments": (resolve)=>{
                                directionsService.route(req, {
                                    "GoogleMapComponent.useEffect.drawDirectionsBySegments": (result, status)=>{
                                        if (window.google.maps.DirectionsStatus && status === window.google.maps.DirectionsStatus.OK || status === 'OK') {
                                            const route = result.routes[0];
                                            const path = route.overview_path.map({
                                                "GoogleMapComponent.useEffect.drawDirectionsBySegments.path": (p)=>({
                                                        lat: p.lat(),
                                                        lng: p.lng()
                                                    })
                                            }["GoogleMapComponent.useEffect.drawDirectionsBySegments.path"]);
                                            allPaths.push(...path);
                                        } else {
                                ...
                                }["GoogleMapComponent.useEffect.drawDirectionsBySegments"]);
                            }
                        }["GoogleMapComponent.useEffect.drawDirectionsBySegments"]);
                    }
            ...
            const addCustomMarkersFromPlan = {
                "GoogleMapComponent.useEffect.addCustomMarkersFromPlan": (routePlan)=>{
                    routePlan.forEach({
                        "GoogleMapComponent.useEffect.addCustomMarkersFromPlan": (point, index)=>{
                            const coords = {
                                lat: point.lat,
                                lng: point.lon
                            };
          ...
          `
                            ...
                            marker.addListener('click', {
                                "GoogleMapComponent.useEffect.addCustomMarkersFromPlan": ()=>{
                                    infoWindow.open(map, marker);
                                }
                            }["GoogleMapComponent.useEffect.addCustomMarkersFromPlan"]);
                        }
